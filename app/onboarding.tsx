...
function PaywallStep({ onSubscribe, onBack }: { onSubscribe: () => void; onBack: () => void }) {
  ...
  const annualPerMonth = annualPkg
    ...
    : "$6.67/mo";
  // Never invent a trial. Defaulting these to 7 advertised "7 days free" even
  // when StoreKit reported no introductory offer, which is exactly the
  // disclosure mismatch guideline 3.1.2(c) rejected 1.0.2 for in May 2026.
    ...
    <View style={styles.paywallContainer}>
      ...
      >
        <View style={styles.paywallHero}>
          {/* Plain View on purpose. This was a TouchableOpacity carrying a
              hidden 5-tap bypass that granted paid access without an IAP
              purchase. Apple inferred it from the binary and rejected under
              guideline 3.1.1 in April 2026. Never make this pressable again;
              test with a Sandbox Apple ID instead. */}
        ...
        <View style={styles.planList}>
          ...
          <PlanCard
            ...
            price={weeklyPrice}
            period="/week"
            subtext={trialText(weeklyTrialDays)}
          />
        </View>
